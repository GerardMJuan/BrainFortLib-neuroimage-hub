...
parser.add_argument("--mat_suffix", type=str, nargs=1, required=True, help='suffix input images')
parser.add_argument("--label_dir", type=str, nargs=1, required=True, help='template dir with all the baseline images')
...
mat_list = [f for f in files_list if fnmatch(f, '*' + args.mat_suffix[0])]
assert mat_list, "List of input matrix is empty"

assert os.path.exists(args.label_dir[0]), "Template file dir found"
...
if args.use_labels is not None:
    lab_list = [f.split(args.mat_suffix[0])[0] + args.use_labels[1] for f in mat_list]
...
for mat_file in mat_list:
    img_name = mat_file.split(args.mat_suffix[0])[0]   

    

    img_path = os.path.join(args.in_dir[0], mat_file)
    ...
    cmdline += ['--input', '{}'.format(args.label_dir[0])]
        
    if args.output_warped_image:
        cmdline += ['--output', '{}LabelWarped.nii.gz'.format(os.path.join(args.out_dir[0], img_name), args.out_warp_intfix[0], os.path.join(args.out_dir[0], mat_file.split(os.extsep, 1)[0] + args.out_warp_intfix[0]))]
    else:
    ...
    cmdline += ['--transform','{}'.format(img_path)]
