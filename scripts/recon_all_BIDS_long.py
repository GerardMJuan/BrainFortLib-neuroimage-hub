...
This script extends recon_all_BIDS, in the sense that it can include
follow-ups for processing.
"""
...
for subject in df_subjects.itertuples():
    ...
    # Continue if scan is not available
    if not subject.bids_status:
        continue

    patient_id = 'ADNI' + subject.ptid[0:3] + 'S' + subject.ptid[6:]
    month = subject.month
    img = layout.get(subject=patient_id, extensions='.nii.gz', modality='anat', session=month)
    print(img)
    img = img[0]
    ...
    name = img.subject + '_' + img.session

    # Depending on fs_status column, do one thing or another
    if subject.fs_status is not 'Done':
        cmdline = ['recon-all', '-all', '-s', name, '-i', img_path, '-sd', out_dir, '-no-isrunning']
    else:
        continue

    print(' '.join(cmdline))
    print("Launching recon_all of file {}".format(img_file))
